...
declare module 'airtable' {
  class Airtable {
    ...
    base<Schema extends {}>(appId: string): Airtable.Base<Schema>
  }
  ...
  namespace Airtable {
    ...
    interface Base<Schema extends FieldSet> {
      (tableName: string): Table<Schema>
    }
    ...
    interface Table<TFields extends FieldSet> {
      ...
      create(
        records: TFields[],
        opts?: { typecast: boolean }
      ): Promise<Array<Response<TFields>>>
    ...
    interface Query<TFields extends object> {
      ...
      eachPage(
        pageCallback: (records: Response<TFields>, next: () => void) => void
      ): Promise<void>
    }
